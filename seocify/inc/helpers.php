...
if ( !function_exists( 'seocify_content_read_more' ) ) {
	...
	function seocify_content_read_more( $num = 20, $button = true ) {
		...
		echo '</div>';
		if ( $button == true ) {
			echo '<div class="btn-wraper"><a href="' . get_the_permalink() . '" class="btn btn-primary icon-right">' . esc_html( $blog_button_title ) . '<i class="xsicon xsicon-arrow-right"></i></a></div>';
		}
	}
