	...
	// Don't print empty markup if there's nowhere to navigate.
		...
		if ( !empty( $pre_post ) ):
			...
			<a href="<?php echo get_the_permalink( $pre_post->ID ); ?>" class="single-post-nav">
				...
				<h3 class="post-nav-title icon-left"><i class="xsicon xsicon-arrow-left"></i> <?php esc_html_e( 'Previous Post', 'seocify' ) ?></h3>
		...
		if ( !empty( $next_post ) ):
			...
			<a href="<?php echo get_the_permalink( $next_post->ID ); ?>" class="single-post-nav">
				...
				<h3 class="post-nav-title icon-right"><i class="xsicon xsicon-arrow-right"></i> <?php esc_html_e( 'Next Post', 'seocify' ) ?></h3>
...
if ( !function_exists( 'seocify_post_meta' ) ) {
	...
	function seocify_post_meta() {
		...
		printf(
			'<span class="post-author"><a href="%1$s"><i class="xsicon xsicon-user"></i> %2$s</a></span>', esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ), get_the_author()
		);
		...
		if ( comments_open() ) :
			echo '<span class="post-comments"><i class="xsicon xsicon-comment-alt"></i> ';
		...
		if ( get_post_type() === 'post' ) {
			...
			if ( !is_single() ) {
				...
				if ( $tag_list ) {
					echo '<span class="tagcloud"><i class="xsicon xsicon-tags"></i> ' . $tag_list . ' </span>';
				}
			}
			if ( is_single() ) {
				// Edit link.
	...
	if ( !function_exists( 'seocify_post_meta_left' ) ) {
		...
		function seocify_post_meta_left() {
			...
			if ( get_post_type() === 'post' ) {
				...
				// Comments link.
				if ( comments_open() ) :
					echo '<span class="post-comment"><i class="xsicon xsicon-comment-alt"></i> ';
					comments_popup_link( esc_html__( '0', 'seocify' ), esc_html__( '1', 'seocify' ), esc_html__( '%', 'seocify' ) );
					echo '</span>';
				endif;
...
if ( !function_exists( 'seocify_paging_nav' ) ) {
	...
	function seocify_paging_nav() {
		...
		if ( get_previous_posts_link() )
			printf( '<li>%s</li>' . "\n", get_previous_posts_link( '<i class="xsicon xsicon-long-arrow-alt-left"></i>' ) );
		...
		if ( get_next_posts_link() )
			printf( '<li>%s</li>' . "\n", get_next_posts_link( '<i class="xsicon xsicon-long-arrow-alt-right"></i>' ) );
...
function seocify_link_pages() {
	$args = array(
		...
		'separator'			 => '  ',
		'nextpagelink'		 => esc_html__( 'Next ', 'seocify' ) . '<I class="xsicon xsicon-chevron-right"></i>',
		'previouspagelink'	 => '<I class="xsicon xsicon-chevron-left"></i>' . esc_html__( ' Previous', 'seocify' ),
	);
	wp_link_pages( $args );
}
